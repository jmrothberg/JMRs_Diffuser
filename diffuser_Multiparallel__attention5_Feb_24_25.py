...
class ConditionalUNet(nn.Module):
    ...
    """
    def __init__(self, num_classes=10, emb_dim=32, timesteps=1000, in_channels=3, use_attention=False):
        super().__init__()
        ...
        init_channels   = int(512 * capacity_mult)   # was 512 originally
        down1_channels  = int(1024 * capacity_mult)  # was 1024
        down2_channels  = int(2048 * capacity_mult)  # was 2048
        up1_channels    = int(1024 * capacity_mult)  # was 1024 in up path
        up2_channels    = int(512 * capacity_mult)   # was 512 in up path
        ...
        if use_attention:
            middle_layers.append(SelfAttention(down2_channels))
        middle_layers.extend([
            nn.Conv2d(down2_channels, down2_channels, kernel_size=3, padding=1),
            nn.GroupNorm(32, down2_channels),
            nn.SiLU(),
        ...
        if use_attention:
            up1_layers.append(SelfAttention(up1_channels))
        up1_layers.extend([
        ...
        if use_attention:
            up2_layers.append(SelfAttention(up2_channels))
        up2_layers.extend([
...
def train(model, diffusion, dataloader, optimizer, device, num_epochs, dataset_name, override_lr=None):
        ...
        if root_checkpoints:
            for ckpt_file in root_checkpoints:
                ...
                if (ckpt.get('dataset_name') == dataset_name and 
                    ...
                    ckpt.get('use_attention', False) == model_unwrapped.use_attention):  # Check attention matches
            ...
            if dir_checkpoints:
                ...
                if (latest_ckpt.get('dataset_name') == dataset_name and 
                    ...
                    latest_ckpt.get('use_attention', False) == model_unwrapped.use_attention):  # Check attention matches
    ...
    if checkpoint_path:
        ...
        is_compatible = (
            ...
            checkpoint.get('use_attention', False) == model_unwrapped.use_attention
        )
